...
public class JDayChooser extends JPanel implements ActionListener, KeyListener, FocusListener {
    ...
    protected boolean drawTrainingWeek; // $ new
	...
	public void drawWeeks() {
        ...
        so.util.SokkerCalendar tmpCalendar = new so.util.SokkerCalendar(); // $ new
		...
		for (int i = 1; i < 7; i++) {
			...
			int week = tmpCalendar.getWeekOfSeason(); // $ new
            ...
            //weeks[i].setToolTipText(tmpCalendar.getSeasonText()); // $ new
            weeks[i].setToolTipText( so.gui.MainFrame.getLabel(so.Constants.Labels.TXT_SEASON) + " " +
                                     Integer.toString(tmpCalendar.getSeason()) ); // $ new
...
//                 if (tmpCalendar.isSameTrainingWeekAs(this.day)) { // $ new
//                     days[i + n + 7].setBackground(oldDayBackgroundColor.brighter()); // $ new
//                 } // $ new
			}
	...
	public void setDay(int d) {
		...
		for (int i = 7; i < 49; i++) {
			if (days[i].getText().equals(Integer.toString(day))) {
				...
				selectedDay.setBorder( javax.swing.BorderFactory.createEtchedBorder(javax.swing.border.EtchedBorder.RAISED) );
				break;
			}
		}

        drawTrainingWeek(); // $ new
    ...
    // $ new BEGIN
    public void drawTrainingWeek() {
        if (drawTrainingWeek) {
            so.util.SokkerCalendar skCalendar = new so.util.SokkerCalendar( calendar.getTime() );
            skCalendar.set(Calendar.DAY_OF_MONTH, 1);
            int maxDaysInMonth = skCalendar.getMaxDaysInMonth();
            int firstDayOfWeek = skCalendar.getFirstDayOfWeek();
            int firstDay = skCalendar.get(Calendar.DAY_OF_WEEK) - firstDayOfWeek;
            if (firstDay < 0) {
                firstDay += 7;
            }
            firstDay += 7;
            /* loopday = i - 6; */
            int endLoop = firstDay + maxDaysInMonth;
            Font normalFont = days[2].getFont();
            Font trainingFont = normalFont.deriveFont(Font.BOLD);
            for (int i=firstDay; i<endLoop; i++) {
                if (skCalendar.isSameTrainingWeekAs(day)) {
                    days[i].setFont(trainingFont);
                    //days[i].setForeground(Color.YELLOW);
                    days[i].setBackground(selectedColor);
                }
                else {
                    days[i].setFont(normalFont);
                    //days[i].setForeground(Color.BLACK);
                    days[i].setBackground(oldDayBackgroundColor);
                }
                skCalendar.add(Calendar.DAY_OF_MONTH, 1);
            }
        }
    }
    public void setDrawTrainingWeek(boolean draw) {
        drawTrainingWeek = draw;
        drawTrainingWeek();
    } // $ new END
